...
import { selectUser, updateUser } from '../Feature/Userslice';
...
function Profile() {
    ...
    const handleAvatarUpdate = (newAvatar) => {
        dispatch(updateUser({ ...user, photo: newAvatar }));
    };
    ...
    return (
        <div className="flex items-center mt-9 mb-4 justify-center">
            <div className="max-w-xs">
                <div className="bg-white shadow-lg rounded-lg py-3">
                    <div className="photo-wrapper p-2">
                        <img src={user.photo} alt="User Avatar" className="w-32 h-32 rounded-full mx-auto" />
                        <button className="btn mt-3" onClick={() => setShowAvatarModal(true)}>Change Avatar</button>
                    </div>
                    <div className="p-2">
            ...
            {showAvatarModal && (
                <UpdateAvatar userId={user.uid} onClose={() => setShowAvatarModal(false)} onUpdate={handleAvatarUpdate} />
            )}
        </div>
    );
}
